...
import { getPets, getPetById, PetSummary } from '../api/petsApi'
...
export default function AdminPage() {
  ...
  const openEditDialog = async (pet: PetSummary) => {
    setEditingId(pet.id)
    setSaveError(null)
    setDialogOpen(true)

    try {
      const detail = await getPetById(pet.id)
      setForm({
        name: detail.name,
      ...
      setPhotos(detail.photos.length > 0
        ? detail.photos.map((photo) => ({ url: photo.url, isPrimary: photo.isPrimary }))
        : [])
    } catch {
      setSaveError('Failed to load pet details for editing.')
    }
  }
  ...
  return (
    <>
      ...
      <Container maxWidth="lg" sx={{ mt: 4, mb: 8 }}>
        <Paper className="glass-card p-5 rounded-[32px] mb-4" elevation={0}>
          <Box display="flex" flexDirection={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems="center" gap={2}>
            <Box>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
                Create, update, and remove pet listings in a sleek modern admin console.
              </Typography>
            </Box>
            <Button variant="contained" startIcon={<AddIcon />} onClick={openAddDialog} sx={{ textTransform: 'none' }}>
              Add New Pet
            </Button>
          </Box>
        </Paper>
        ...
        <TableContainer component={Paper} className="glass-card" sx={{ borderRadius: 4, overflow: 'hidden' }}>
          <Table>
            <TableHead>
              <TableRow>
                ...
                : pets.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={8} align="center" sx={{ py: 6 }}>
                      No pets available yet. Use the Add New Pet button to create your first entry.
                    </TableCell>
                  </TableRow>
                ) : pets.map((pet) => (
